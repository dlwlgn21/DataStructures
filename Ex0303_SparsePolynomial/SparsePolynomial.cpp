...
#include "..\Ex0302_Polynomial\Polynomial.h"
#include <iostream>
#include <cassert>
#include <algorithm>
...
SparsePolynomial::SparsePolynomial(const SparsePolynomial& other)
{
	...
	{
		terms_[i].coef = other.terms_[i].coef;
		terms_[i].exp = other.terms_[i].exp;
	}
}
...
SparsePolynomial SparsePolynomial::Add(const SparsePolynomial& poly)
{
	...
	while (true)
	{
		if (i >= num_terms_ && j >= poly.num_terms_)
			break;

		if (i >= num_terms_)
		{
			for (int jj = j; jj < poly.num_terms_; ++jj)
			{
				temp.NewTerm(poly.terms_[jj].coef, poly.terms_[jj].exp);
			}
			break;
		}
		if (j >= poly.num_terms_)
		{
			for (int ii = i; ii < num_terms_; ++ii)
			{
				temp.NewTerm(terms_[ii].coef, terms_[ii].exp);
			}
			break;
		}
		const Term& a = terms_[i];
		const Term& b = poly.terms_[j];
		
		if (a.exp < b.exp)
		{
			temp.NewTerm(a.coef, a.exp);
			++i;
		}
		else if (a.exp > b.exp)
		{
			temp.NewTerm(b.coef, b.exp);
			++j;
		}
		else
		{
			temp.NewTerm(a.coef + b.coef, a.exp);
			++i;
			++j;
		}
	}
