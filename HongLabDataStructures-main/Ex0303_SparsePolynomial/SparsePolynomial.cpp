...
{
	this->num_terms_ = other.num_terms_;
	this->terms_ = new Term[this->num_terms_];
	for (int i = 0; i < this->num_terms_; ++i)
	{
		this->terms_[i].coef = other.terms_[i].coef;
		this->terms_[i].exp = other.terms_[i].exp;
	}
}
...
float SparsePolynomial::Eval(float x)
{
	...
	{
		temp += terms_[i].coef * std::powf(x, static_cast<float>(terms_[i].exp));
	}
	return temp;
}
...
SparsePolynomial SparsePolynomial::Add(const SparsePolynomial& other)
{
	...
	SparsePolynomial retNew;

	// TODO:
	int i = 0;
	int j = 0;
	int thisMaxNumTerm = this->num_terms_;
	int otherMaxNumTerm = other.num_terms_;

	while (true)
	{
		if (i >= thisMaxNumTerm || j >= otherMaxNumTerm)
		{
			break;
		}

		int thisExpo = this->terms_[i].exp;
		int otherExpo = other.terms_[j].exp;
		float thisCoef = terms_[i].coef;
		float otherCoef = other.terms_[j].coef;
		if (thisExpo == otherExpo)
		{
			retNew.NewTerm(thisCoef + otherCoef, thisExpo);
			++i;
			++j;
		}
		else if (thisExpo < otherExpo)
		{
			retNew.NewTerm(thisCoef, thisExpo);
			++i;
		}
		else
		{
			retNew.NewTerm(otherCoef, otherExpo);
			++j;
		}
	}

	for (; i < thisMaxNumTerm; ++i)
	{
		retNew.NewTerm(terms_[i].coef, terms_[i].exp);
	}

	for (; j < otherMaxNumTerm; ++j)
	{
		retNew.NewTerm(other.terms_[j].coef, other.terms_[j].exp);
	}
	return retNew;
}
